...
public class ModOptions {
    ...
    static {
        ...
        colorValue = CLIENT.createAccessor((v) -> CLIENT.set(c -> c.color = getColorFromFormatting(v)), () -> getFormattingFromString(CLIENT.get(getColorFromFormatting(ChatFormatting.GOLD), c -> c.color)));
    ...
    private static String getColorFromFormatting(ChatFormatting color) {
        if (!color.isColor()) {
            return getColorFromFormatting(ChatFormatting.GOLD);
        }

        return color.getName().toLowerCase().replaceAll("[^a-z]", "");
    }

    private static ChatFormatting getFormattingFromString(String color) {
        ChatFormatting formatting = ChatFormatting.getByName(color);
        if (formatting == null || !formatting.isColor()) return ChatFormatting.GOLD;
        return formatting;
    }
